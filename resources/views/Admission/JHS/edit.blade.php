...
<div class="container-fluid px-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap">
        ...
        <a href="{{ route('admission.jhs.index') }}" class="btn btn-outline-secondary btn-sm mt-2 mt-md-0">
            <i class="bi-chevron-left"></i> Back
        </a>
    </div><br>

    <form action="{{ route('admission.jhs.update', $application->application_number) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="row g-3 mb-4">
            <div class="col-12">
                <label class="form-label">JHS Program:</label>
                <select class="form-select" name="program" required>
                    @foreach($programs as $strand)
                        <option value="{{ $strand->id }}"
                            {{ old('choice_first', $application->choice_first) == $strand->id ? 'selected' : '' }}>
                            JHS: {{ $strand->code }} - {{ $strand->name }}
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="row g-3 mt-1">
            <div class="col-12 col-md-3">
                ...
                <select class="form-select" name="applicant_status" required>
                    <option value="Incoming Grade 7" {{ old('applicant_status', $application->applicant_status ?? '') == 'Incoming Grade 7' ? 'selected' : '' }}>Incoming Grade 7</option>
                    <option value="Grade 8" {{ old('applicant_status', $application->applicant_status ?? '') == 'Grade 8' ? 'selected' : '' }}>Grade 8</option>
                    <option value="Grade 9" {{ old('applicant_status', $application->applicant_status ?? '') == 'Grade 9' ? 'selected' : '' }}>Grade 9</option>
                    <option value="Grade 10" {{ old('applicant_status', $application->applicant_status ?? '') == 'Grade 10' ? 'selected' : '' }}>Grade 10</option>
                    </select>
            </div>
            <div class="col-12 col-md-3">
            ...
            <div class="row mb-4">
                <div class="col-6">
                    <div class="table-responsive">
                        @foreach($subtests as $groupKey => $groupSubtests)
                            @if($groupKey == 1)
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @foreach($groupSubtests as $subtest)
                                            <tr class="freshman">
                                                <td>{{ $subtest->name }}</td>
                                                <td colspan="2"><input class="form-control text-center" type="text"name="rs[{{ $subtest->id }}]"value="{{ $subtest->result->rs ?? '' }}" required></td>
                                                <td colspan="2"><input class="form-control text-center" type="text"name="hg[{{ $subtest->id }}]"value="{{ $subtest->result->hg ?? '' }}" required></td>
                                            </tr>
                                        @endforeach 
                                    </tbody>
                                </table>
