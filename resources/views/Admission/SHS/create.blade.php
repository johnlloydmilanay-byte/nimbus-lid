        ...
        <div class="row g-3 mb-4">
            ...
            <div class="col-12">
                ...
                <select class="form-select" name="choice_second" required>
                    @foreach($programs as $key => $strand)
                        <option value="{{ $strand->id }}" {{ $key === 1 ? 'selected' : '' }}>
                            {{ $strand->code }} - {{ $strand->name }}
                        </option>
                    @endforeach
                </select>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function searchStudents(query) {
            fetch(`{{ route('admission.shs.search-students') }}?query=${encodeURIComponent(query)}`)
        ...
        function selectStudent(studentId) {
            fetch(`{{ route('admission.shs.student-details', '') }}/${studentId}`)
                .then(response => response.json())
                .then(student => {
